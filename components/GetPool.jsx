          ...
          <div className="grid lg:grid-cols-2 gap-10">
            ...
            <div className="hidden lg:block ps-4 py-4">
              <div className="relative rounded-xl overflow-hidden h-full w-full">
                <img src="assets/images/ai/auth-img.jpg" alt="auth-img" className="w-full h-full transform -scale-x-100" />
                ...
                <div className="pb-6 my-auto">
                  ...
                  <div className="text-start">
                    <div className="mb-4">
                      ...
                      <input type="text" onChange={(e) => handleFormFieldChange('fee', e)} className="block w-full rounded py-1.5 px-3 bg-transparent border-white/10 border-default-200 text-white/80 focus:border-white/25 focus:ring-transparent" placeholder="Enter the pool fee" />
                    </div>
                  </div>

                  <div className="mb-6 text-center">
                    <button onClick={() => CALL_POOL_ADDRESS()} className="w-full inline-flex items-center justify-center py-6 px-6 backdrop-blur-2xl bg-primary-600/90 text-white rounded-lg transition-all duration-500 group hover:bg-primary-600 mt-5">
...

                  {
                    poolAddress && (
                      ...
                      {poolAddress.map((pool, index) => (
                        <div className="mb-4">
                          <input className="block w-full rounded py-1.5 px-3 bg-transparent border-white/10 border-default-200 text-white/80 focus:border-white/25 focus:ring-transparent" key={index+2} type="text" onClick={(e) => navigator.clipboard.writeText(pool)} value={`${index+1} : ${pool} `} />
                        </div>
                      ))}
                      </>
                    )}
